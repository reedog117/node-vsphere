...
var schemaObjectSpec = Joi.object({
  ...
  skip: Joi.boolean(),
  selectSet: Joi.array().includes( schemaSelectionSpec, schemaTraversalSpec )
});
...
var schemaPropertySpec = Joi.object({
  ...
  attributes: Joi.object().required(),
  all: Joi.boolean(),
  pathSet: Joi.array().includes( Joi.string() ),
  type: Joi.string().valid( ManagedObjectTypes ).required(),
});
...
var schemaPropertyFilterSpec = Joi.object({
  ...
  attributes: Joi.object().required(),
  objectSet: Joi.array().min(1).includes( schemaObjectSpec ).required(),
  propSet: Joi.array().min(1).includes( schemaPropertySpec ).required(),
