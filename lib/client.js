...
Client.prototype.powerOpVMByMORef = function( MORefs, powerOp ) {
  ...
  if( typeof MORefs == 'undefined' || _.isEmpty(MORefs)) {
    powerOp = null;
  ...
  if( powerCommand ) {
    ...
    if( MORefs && !MORefs.length ) {
      self.runCommand(powerCommand, { _this: MORefs })
      .once('result', function(result){
        ...
        .once('result', function( result ) {
          if(result['info.error'] == undefined) {
            emitter.emit('result', { obj: MORefs, result: result['info.state'] });
          } else {
            emitter.emit('error', { obj: MORefs, result: result['info.error'] });
          }
        })
        .once('error', function( err ) {
          console.error(err);
